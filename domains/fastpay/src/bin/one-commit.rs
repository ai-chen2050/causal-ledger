...
#[structopt(
    name = "FastPay Once-Commit Benchmark",
...
impl ClientServerBenchmark {
    ...
    async fn launch_client(&self, mut orders: Vec<(u32, Bytes)>) {
        ...
        // order txs just needs once commit
        ...
        if self.max_in_flight > 0 {
            ...
            for resp in responses {
                let result = deserialize_response(resp.bytes());
                debug!("Received resp: {:?}", result);
            }
        } else {
